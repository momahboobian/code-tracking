# cda - Activity Summary 

## Overall Statistics

| Stat                   | Value                                                             |
| ---------------------- | ----------------------------------------------------------------- |
| **Lines Added** (➕)   | 15547                                          |
| **Lines Removed** (➖) | 362                                        |
| **Net Change** (↕)    | 15185                |
| **Active Time** (⌚)   | 155 minutes |


## Modified Files
- **fieldUtils.ts** (+407, -6)
- **AttachmentDetailsPanel.test.tsx** (+423, -143)
- **AttachmentDetailsPanel.tsx** (+103, -12)
- **Panel.scss** (+12, -0)
- **ProfilePublic.tsx** (+209, -9)
- **Profile.types.ts** (+654, -42)
- **graphql.ts** (+8290, -123)
- **20260317142951-replace-peopleview-teams-view.js** (+60, -0)
- **profile.js** (+241, -4)
- **team.js** (+141, -2)
- **team.test.js** (+493, -10)
- **20260202163922-replace-peopleview-profiles-view.js** (+128, -0)
- **20260317154110-dsf.js** (+128, -0)
- **sap_views.ts** (+1470, -0)
- **profile.test.js** (+853, -4)
- **peopleview.js** (+424, -1)
- **profileFieldsConfig.ts** (+499, -6)
- **ProfileFields.types.ts** (+118, -0)
- **queries.ts** (+782, -0)
- **gql.ts** (+112, -0)

## Visualizations

### By File Type (Lines Changed)

```mermaid
pie showData
title Lines changed by file type
".ts" : 12509
".tsx" : 899
".scss" : 12
".js" : 2489
```

### By Hour (Estimated Activity Count)

```mermaid
pie showData
title Coding activity by hour (count of changes)
"10h" : 21
"11h" : 3
"12h" : 10
"14h" : 7
"15h" : 16
"16h" : 32
"17h" : 24
```


> **Last Updated:** 17/03/2026, 17:45:31