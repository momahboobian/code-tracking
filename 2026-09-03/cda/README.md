# cda - Activity Summary 

## Overall Statistics

| Stat                   | Value                                                             |
| ---------------------- | ----------------------------------------------------------------- |
| **Lines Added** (➕)   | 75992                                          |
| **Lines Removed** (➖) | 25834                                        |
| **Net Change** (↕)    | 50158                |
| **Active Time** (⌚)   | 436 minutes |


## Modified Files
- **package.json** (+266, -44)
- **App.tsx** (+319, -0)
- **tsconfig.json** (+29, -0)
- **GroupDetails.scss** (+97, -0)
- **GroupDetails.test.tsx** (+472, -0)
- **App.tsx** (+433, -4)
- **GroupDetails.tsx** (+444, -0)
- **queries.js** (+844, -0)
- **skill-export.test.ts** (+741, -0)
- **skill-queries.ts** (+1215, -0)
- **SkillGroups.test.ts** (+1704, -0)
- **package.json** (+128, -0)
- **Groups.tsx** (+97, -0)
- **yarn.lock** (+14168, -0)
- **vite.config.ts** (+44, -0)
- **declarations.d.ts** (+2624, -2136)
- **skills.js** (+459, -0)
- **skills.ts** (+386, -0)
- **transform-group-skill-progress.test.ts** (+152, -0)
- **GroupMembersList.tsx** (+76, -0)
- **SortableTable.tsx** (+109, -0)
- **GroupSkillProgress.tsx** (+133, -0)
- **GroupMembersView.tsx** (+14, -0)
- **GroupSkillProgress.scss** (+8, -0)
- **index.ts** (+2, -0)
- **index.ts** (+2, -0)
- **SortableTable.scss** (+15, -0)
- **SortableTable.test.tsx** (+41, -0)
- **GroupSkillProgress.test.tsx** (+82, -0)
- **SkillExplore.tsx** (+297, -0)
- **tsconfig.node.json** (+21, -0)
- **tsconfig.app.json** (+23, -0)
- **index.html** (+27, -0)
- **gitVersion.cjs** (+21, -0)
- **eslint.config.js** (+33, -0)
- **codegen.ts** (+32, -0)
- **.gitignore** (+4, -0)
- **.env.development** (+5, -1)
- **setupTests.ts** (+17, -0)
- **index.tsx** (+20, -0)
- **index.scss** (+2, -0)
- **Sidebar.test.tsx** (+86, -15)
- **SearchUser.tsx** (+76, -1)
- **SearchUser.tsx** (+102, -0)
- **graphql.ts** (+18469, -8733)
- **yarn.lock** (+27826, -14673)
- **package.json** (+187, -0)
- **GroupManagement.stories.tsx** (+624, -88)
- **CalendarShare.tsx** (+398, -5)
- **CalendarShareForm.tsx** (+62, -10)
- **ConfirmationModal.tsx** (+146, -5)
- **Duty.tsx** (+778, -5)
- **Settings.tsx** (+78, -5)
- **SignInStatusIcon.tsx** (+204, -5)
- **Admin.tsx** (+66, -5)
- **ProfileBadge.tsx** (+162, -5)
- **TeamViewRow.tsx** (+338, -5)
- **SchedulingTeamSelect.tsx** (+138, -0)
- **TooltipBadge.tsx** (+136, -10)
- **DateSwitcher.tsx** (+228, -15)
- **ise-web-components.d.ts** (+17, -0)
- **Sidebar.tsx** (+203, -8)
- **generate-holidays.cjs** (+62, -56)

## Visualizations

### By File Type (Lines Changed)

```mermaid
pie showData
title Lines changed by file type
".json" : 698
".tsx" : 6553
".scss" : 122
".js" : 1336
".ts" : 36274
".lock" : 56667
".html" : 27
".cjs" : 139
"unknown" : 4
".development" : 6
```

### By Hour (Estimated Activity Count)

```mermaid
pie showData
title Coding activity by hour (count of changes)
"10h" : 6
"11h" : 18
"12h" : 32
"13h" : 26
"14h" : 27
"16h" : 108
"17h" : 86
```


> **Last Updated:** 03/09/2026, 17:38:50